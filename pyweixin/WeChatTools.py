...
class Tools():
    ...
    @staticmethod
    def find_profile_popup(parent_window: WindowSpecification | None = None, timeout: float = 2) -> WindowSpecification:
        """Locate the profile popup across different WeChat 4.1 window layouts."""
        popup_spec = dict(Windows.PopUpProfileWindow)
        candidates = []
        if parent_window is not None:
            candidates.append(parent_window.window(**popup_spec))
        candidates.append(desktop.window(**popup_spec))
...
class Navigator():
        ...
        #如果当前主界面聊天界面顶部的名称为好友名称，is_find为True,直接返回此时主界面
            ...
            if listItems:
                ...
                for _ in range(search_pages):
                    time.sleep(0.1)
                    search_result=select_in_messageList(friend)
                    friend_button=search_result[0] if isinstance(search_result, tuple) else search_result
    ...
    def open_friend_profile(friend:str,is_maximize:bool=None,search_pages:int=None)->tuple[WindowSpecification,WindowSpecification]:
        ...
        if friend_button.exists(timeout=1):
            ...
            profile_pane=Tools.find_profile_popup(parent_window=main_window)
